...
final class CartViewModel {
    ...
    func applySavedSortOption() {
        if let savedOption = loadSortOption() {
            ...
            Logger.log(
                "No saved sort option found. Using default: \(String(localizable: .sortNftName))",
                level: .debug)
        }
    }
            ...
            DispatchQueue.main.async {
                ...
                case .failure(let error):
                    if let cartError = error as? NftServiceError {
                        let errorMessages = cartError.errors.map { $0.localizedDescription }.joined(
                            separator: "\n")
    ...
    private func loadSortOption() -> String? {
        return UserDefaults.standard.string(forKey: sortOptionKey)
