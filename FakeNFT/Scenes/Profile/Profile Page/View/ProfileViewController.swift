...
final class ProfileViewController: UIViewController {
    ...
    init(servicesAssembly: ServicesAssembly) {
        self.viewModel = ProfileViewModel(
            profileNetworkService: servicesAssembly.profileNetworkService)
                ...
                self.avatarImageView.kf.setImage(
                    ...
                    completionHandler: { result in
                        ...
                        case .success(let value):
                            Logger.log(
                                "Image successfully loaded: \(value.source.url?.absoluteString ?? "")",
                                level: .debug)
                        case .failure(let error):
                            Logger.log(
                                "Error loading image: \(error.localizedDescription)", level: .error)
                        }
                    })
            } else {
    ...
    @objc private func openUserWebsite() {
        guard let viewModel = viewModel,
            let website = viewModel.userProfile?.website
        else { return }
