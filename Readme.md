...
1. Использовать macOS Ventura 13.5 или выше
2. Установить Xcode 15 или выше
3. Склонировать репозиторий в любую директорию
4. Открыть проект в Xcode
5. Указать токен в файле FakeNFT/Services/Requests/RequestConstants.swift
6. Выбрать устройство для запуска приложения
7. Нажать на кнопку запуска
8. Если появится ошибка "Cannot infer contextual base in reference to member" открыть список ошибок, найти ошибку пакета xcstrings-tool и выбрать "Trust", после этого снова запустить проект. Если не получается найти ошибку, то попробуйте сделать полную чистку проекта и заново собрать.
